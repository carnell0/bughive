...
const columnConfig = {
  ...
  in_progress: {
    label: "In Progress",
    count_color: "bg-blue-100 text-blue-600",
  },
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xs font-semibold text-gray-700 uppercase tracking-wide whitespace-nowrap">
          {title}
        </h2>
        <span
          className={`text-xs font-medium px-2 py-0.5 rounded-full ${columnConfig[id as keyof typeof columnConfig]?.count_color}`}
        >
          {tickets.length}
        </span>
      </div>
